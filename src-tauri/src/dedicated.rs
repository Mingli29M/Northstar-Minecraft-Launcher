...
impl Default for DedicatedProperties {
    fn default() -> Self {
        Self {
            motd: "Northstar Dedicated Server".into(),
            max_players: 20,
...
pub fn get_dedicated_properties(id: &str) -> Result<DedicatedProperties, String> {
    ...
    Ok(DedicatedProperties {
        motd: map
            .get("motd")
            .cloned()
            .unwrap_or_else(|| "Northstar Dedicated Server".into()),
        max_players: map
            .get("max-players")
...
fn serialize_properties(map: &HashMap<String, String>) -> String {
    ...
    keys.sort();
    let mut out = String::from("# Minecraft server properties — managed by Northstar\n");
    for k in keys {
...
fn install_forge_server(server: &mut HostServer, runtime: &Path) -> Result<(), String> {
    ...
    if platform_args.exists() {
        ...
        if !jvm_args.exists() {
            fs::write(
                &jvm_args,
                "# JVM args managed by Northstar on start\n-Xmx2G\n",
            )
            .map_err(|e| e.to_string())?;
        }
        write_launch_marker(
...
fn install_neoforge_server(server: &mut HostServer, runtime: &Path) -> Result<(), String> {
    ...
    if platform_args.exists() {
        let jvm_args = runtime.join("user_jvm_args.txt");
        if !jvm_args.exists() {
            fs::write(&jvm_args, "# JVM args managed by Northstar on start\n-Xmx2G\n")
                .map_err(|e| e.to_string())?;
        }
        write_launch_marker(
            runtime,
